...
from .collector import CiscoCollector
...
# cisco_ios. device_type is forced to this value so the netmiko driver can
# never diverge from the parsers, regardless of what a client sends.
SERVER_DEVICE_TYPE = "cisco_ios"

# ---- Global state ----
CONFIG: dict = {}
SESSION_TTL = 300  # seconds without WS clients before reaping
ssh_lock: asyncio.Lock | None = None  # Serialize SSH to old IOS
...
def load_config() -> dict:
    ...
    return load_yaml_config("cisco.yaml", _example)
...
async def _poll_loop(session_id: str):
    ...
    while True:
        ...
        try:
            ...
            logger.info(
                f"[{session_id[:8]}] Broadcasting "
                f"{len(payload)//1024}KB to {len(session.clients)} client(s)"
            )
...
async def lifespan(app: FastAPI):
    global CONFIG, ssh_lock
    CONFIG = load_config()
    ssh_lock = asyncio.Lock()
    reap_task = asyncio.create_task(_reap_sessions())
    logger.info("Cisco HUD server started (awaiting /api/connect)")
    yield
    reap_task.cancel()
    for sid, s in list(sessions.items()):
        ...
        try:
            ...
            pass
    sessions.clear()
    logger.info("Cisco HUD server stopped")


app = FastAPI(title="Cisco IOS HUD", lifespan=lifespan)
...
async def connect_device(body: dict = Body(...)):
    ...
    collector = CiscoCollector(new_dev)
    try:
        loop = asyncio.get_running_loop()
...
def _open_ssh_shell(cfg: dict, cols: int = 120, rows: int = 36):
    ...
    # Only use key_file when use_keys is true (matches collector behavior)
    ...
    if cfg.get("legacy_ssh", False):
        # IOS 12.x: disable modern pubkey algos AND allow legacy KEX/ciphers
        connect_kwargs["disabled_algorithms"] = {
            "pubkeys": ["rsa-sha2-256", "rsa-sha2-512"],
        }
        # Force legacy algorithms on the Transport
        transport = paramiko.Transport((cfg["host"], cfg.get("port", 22)))
    ...
    # Use vt100 for ancient IOS that may not handle xterm-256color
    term_type = "vt100" if cfg.get("legacy_ssh", False) else "xterm-256color"
    channel = client.invoke_shell(
        term=term_type, width=cols, height=rows
    )
    channel.settimeout(0.05)

    # Auto-enable if secret is configured
...
async def terminal_ws(ws: WebSocket, session: str | None = Query(None)):
    ...
    # Retry with lock — old IOS can't handle concurrent SSH negotiations
    max_retries = 3
    for attempt in range(max_retries):
        try:
            ...
            logger.warning(f"Terminal SSH attempt {attempt+1}/{max_retries} failed: {e}")
            if attempt < max_retries - 1:
                await ws.send_text(f"\r\n\x1b[33mRetrying SSH connection ({attempt+2}/{max_retries})...\x1b[0m\r\n")
                await asyncio.sleep(2)
            else:
                logger.error(f"Terminal SSH connect failed after {max_retries} attempts: {e}")
